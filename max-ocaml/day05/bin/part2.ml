...
let min_in_range (seed_start, range) =
  let rec internal seed_val acc =
    if seed_val < seed_start + range then
      let location = get_location seed_val in
      internal (seed_val + 1) (min location acc)
    else acc
  in
  internal (seed_start + 1) (get_location seed_start)


let seed_locations = List.mapi seed_ranges ~f:(fun i range -> 
  let () = print_endline (String.concat ["Procesing "; (string_of_int i)] ) in
  min_in_range range
  )
